...
int minHeight(Node<int>* node){
    if (node->left == nullptr && node->right == nullptr) return 0;
    else if(node->left == nullptr) return minHeight(node->right)+1;
    else if(node->right == nullptr) return minHeight(node->left)+1;
...
int main(){
    ...
    cout<<max(minHeight(tree.root),0);
